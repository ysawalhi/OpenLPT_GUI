...
class RefractionVSCOptimizer:
    ...
    def optimize_all_cameras(
        ...
        # Pre-build camera-wise batch inputs: each camera evaluates LOS once
        # per residual call with all its observed 2D points.
    ...
    def _residuals(self, x: np.ndarray) -> np.ndarray:
        ...
        for layout_idx, (corr, cam_ids, wids) in enumerate(self._layouts):
            ...
            # Side barrier per unique window (not per camera).
        ...
        if self._nfev_count % 10 == 0:
            ...
            self._log(
                ...
                f"ray_rmse={ray_rmse:.4f}mm violations={violations}"
            )
